...
void LoginWindow::on_LoginButton_clicked()
{
    ...
    QFile staffFile("C:\\Users\\Sama\\Downloads\\System (2)\\System (2)\\System (1)\\HospitalSystem\\staffdata.txt");
    ...
    for (const auto& key : patients.keys()) {
        if (patients.value(key).getUsername() == username && patients.value(key).getPassword() == password) {
            ...
            hide();
            PatientDashboard* patientWindow = new PatientDashboard(username, patients); // Allocate memory
            patientWindow->show();
            validCredentials=true;
            return; // Exit the function early
        }
